...
function getCellStyle(value, isLoop, isDiagonal, isAssignment) {
  ...
  if (isAssignment)
    return { bg: "#022c22", text: "#6ee7b7", shadow: "inset 0 0 0 2px #10b981" };
...
export default function AdjacencyMatrix() {
    ...
    const { nodes, edges, assignmentResult } = location.state ?? { nodes: [], edges: [] };
    ...
    <div style={styles.page}>
      ...
      {assignmentResult && (
        <div style={styles.assignmentBanner}>
          <div style={styles.assignmentTitle}>Solución de asignación (actual en el grafo)</div>
          <p style={styles.assignmentText}>
            <strong style={{ color: assignmentResult.mode === "assignment-min" ? "#60a5fa" : "#a78bfa" }}>
              {assignmentResult.mode === "assignment-min" ? "Minimización" : "Maximización"}
            </strong>
            {" · "}
            Valor óptimo:{" "}
          ...
          {assignmentResult.hasMultipleSolutions && nAlt > 1 && (
            <p style={styles.assignmentMulti}>
              Existen <strong>{nAlt}</strong> soluciones óptimas distintas con el mismo valor. En el grafo y aquí se
              muestra una de ellas (celdas y aristas resaltadas en verde).
            </p>
          )}
          <p style={styles.assignmentHint}>
            Leyenda: celdas <span style={{ color: "#34d399", fontWeight: 700 }}>verdes</span> = pares recurso–tarea de la asignación elegida.
          </p>
        </div>
      )}
        ...
        <div style={styles.matrixScroll}>
          <table style={styles.table}>
            ...
            <tr>
                ...
                {nodes.map((col) => (
                <th key={col.id} style={styles.headerCell}>{col.label==null ? col.id : col.label}</th>
                ))}
                <th>∑</th>
                <th>Cont Filas</th>
            </tr>
            ...
            {nodes.map((row) => (
                <tr key={row.id}>
                <td style={styles.rowHeaderCell}>{row.label==null ? row.id : row.label}</td>
                {nodes.map((col) => {
                    const val = matrix[row.id][col.id];
                    const isDiag = row.id === col.id;
                    const isLoop = isDiag && val !== null;
                    const isAssignment = assignmentPairKeys.has(`${row.id}|${col.id}`);
                    const { bg, text, shadow } = getCellStyle(val, isLoop, isDiag, isAssignment);
                    return (
                    <td
                        key={col.id}
            ...
            <tr>
              ...
              <td style={{ ...styles.cell, background: "#1e293b", color: "#22d3ee", fontWeight: 700 }} />
              <td style={{ ...styles.cell, background: "#1e293b", color: "#22d3ee", fontWeight: 700 }} />
            </tr>
            </tbody>
          </table>
...
const styles = {
  ...
  assignmentBanner: {
    ...
    padding: "16px 20px",
    borderRadius: 12,
    background: "linear-gradient(135deg, rgba(5,150,105,0.12), rgba(29,78,216,0.08))",
